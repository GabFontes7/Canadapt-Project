...
select
    job_id as vaga_id,
    coalesce(source, 'adzuna') as fonte_vaga,
    coalesce(source_job_id, job_id) as id_vaga_na_fonte,
    coalesce(source_site, source, 'Adzuna') as site_origem,
    ...
    cast(focus_area as varchar) as area_foco_coleta,
    cast(mobility_signals as varchar) as sinais_mobilidade,
    cast(filter_version as varchar) as versao_filtro_coleta,
    cast(job_type as varchar) as tipo_contrato,
    cast(salary_raw as varchar) as salario_texto_original,
    ...
    coalesce(cast(pipeline_run_id as varchar), 'unknown') as pipeline_run_id,
    coalesce(cast(data_contract_version as varchar), 'pre-contract') as data_contract_version,
    coalesce(cast(silver_jobs_schema_version as varchar), 'unknown') as silver_jobs_schema_version,
    coalesce(cast(geo_prompt_version as varchar), 'unknown') as geo_prompt_version,
